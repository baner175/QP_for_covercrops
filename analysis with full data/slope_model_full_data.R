...
m0 <- lmer(b ~ COVERCROP + (1|YEAR:SITE),
           data = df)
summary(m0)
car::Anova(m0, test = 'F')
fixef(m0)
...
plot(b~COVERCROP, data = df,
     main = 'Distribution of linear term across different covercrops',
     xlab = 'Covercrop', ylab = 'Linear term',
     col = c('darkgreen', 'blue', 'orange'))


# fixed: cover crop + site; random: year
m1 <- lmer(b ~ COVERCROP + SITE + (1|YEAR),
           data = df)
summary(m1)
car::Anova(m1, test = 'F')
fixef(m1)